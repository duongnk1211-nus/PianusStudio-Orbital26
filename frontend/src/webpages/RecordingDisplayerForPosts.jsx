...
export default function RecordingDisplayerForPosts() {
  const authChecked = useRequireAuth();

  const navigate = useNavigate();
  const location = useLocation();
  const recordUrl = location.state?.focus ?? null;
  ...
  useEffect(() => {
    ...
    async function loadRecord() {
      ...
      try {
        ...
        const data = await apiFetch(`/record-by-id/${recordUrl}`, {
          method: 'GET',
          headers: { Authorization: `Bearer ${session.access_token}` },
        });
        const raw = data?.record;
        if (!raw) {
          if (!cancelled) setFetchError("User might have deleted the recording.");
          return;
        }
        if (!cancelled) setP(new Record(raw.duration, raw.actions ?? []));
      } catch (err) {
        console.error(`Record ID might not be available right now:`, err);
  ...
  if (fetchError) {
    return (
      <div>
        ...
        <p>{'Record not found or unavailable.'}</p>
      </div>
    );
  }
